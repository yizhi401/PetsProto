...
import cn.peterchen.imtest.xmpp.tool.ArrayStringSetting;
 ...
 * 
 *         In order to work flawlessly with the BackupAgent ALL settings in
 *         SettingsManager have to be of the same type as within the
 *         SharedPreferences back-end AND they need to have the same name
 * 
 */
public class SettingsManager {

	private static final String[] xmppConnectionSettings = { "serverHost",
			"serviceName", "serverPort", "login", "password",
			"useDifferentAccount", "xmppSecurityMode", "useCompression" };
	...
	private ArrayStringSetting _blockedResourcePrefixes = new ArrayStringSetting(
	...
	private ArrayStringSetting _notifiedAddresses = new ArrayStringSetting(
			"notifiedAddress", this);
	...
	/**
	 * Checks if the given fromJid is part of the notified Address set. fromJid
	 * can either be a fullJid or a bareJid
	 * 
	 * @param fromJid
	 *            The JID we received a message from
	 * @return true if the given JID is part of the notified Address set,
	 *         otherwise false
	 */
	...
	public void setConnectOnMainScreenStartup(boolean value) {
		_connectOnMainScreenStartup = saveSetting("connectOnMainscreenShow",
				value);
	}
	...
	private ArrayStringSetting _notifHiddenApps = new ArrayStringSetting(
			"hiddenNotifications", "#sep#", this);

	public ArrayStringSetting getNotifHiddenApps() {
		return _notifHiddenApps;
	}

	private ArrayStringSetting _notifHiddenMsg = new ArrayStringSetting(
			"hiddenMsgNotifications", "#sep#", this);
	...
	private void importPreferences() {
		...
		xmppSecurityMode = getString("xmppSecurityMode", "dis");
		//Because of unknown reason(might because of the server) here we must disable the security mode
		...
		notifyApplicationConnection = getBoolean("notifyApplicationConnection",
				false);
		notifyBattery = getBoolean("notifyBattery", false);
		notifyBatteryInStatus = getBoolean("notifyBatteryInStatus", true);
		batteryNotificationInterval = getString("batteryNotificationInterval",
				"10");
		batteryNotificationIntervalInt = Integer
				.parseInt(batteryNotificationInterval);
		...
		mucServer = getString("mucServer", "");
		String notificationIncomingSmsType = getString(
		...
		_connectOnMainScreenStartup = getBoolean("connectOnMainscreenShow",
				false);
		...
		publicIntentTokenRequired = getBoolean("publicIntentTokenRequired",
				false);
