...
public class XmppManager {
	...
	// Indicates the current action associated to the status (connect, identify,
	// wait X seconds,...)
	...
	private final Runnable mReconnectRunnable = new Runnable() {
		public void run() {
			Log.i("attempting reconnection by issuing intent "
                    + MainService.ACTION_CONNECT);
	...
	private XmppManager(Context context) {
		...
		SmackConfiguration
				.setDefaultParsingExceptionCallback(new ParsingExceptionCallback() {
					@Override
					public void handleUnparsablePacket(
							UnparsablePacket stanzaData) throws Exception {
						Log.e("Handling unparsable Packet. Reconnecting",
								stanzaData.getParsingException());
					}
				});
	...
	/**
	 * This getter creates the XmppManager and init the XmppManager with a new
	 * connection with the current preferences.
	 * 
	 * @param ctx
	 * @return
	 */
	...
	private void start(int initialState) {
		...
		case WAITING_FOR_NETWORK:
			...
			break;
		default:
			throw new IllegalStateException("xmppMgr start() Invalid State: "
					+ initialState);
		}
	}

	/**
	 * calls cleanupConnection and sets _status to DISCONNECTED
	 */
	private void stop() {
	 ...
	 * 
	 * Spawns a new disconnect runnable if the connection is still connected and
	 * removes packetListeners and Callbacks for the reconnectHandler.
	 * 
	 * synchronized because cleanupConnection() -> maybeStartReconnect() ->
	 * connectionClosedOnError() is called from a different thread
	 */
	...
	/**
	 * This method *requests* a state change - what state things actually wind
	 * up in is impossible to know (eg, a request to connect may wind up with a
	 * state of CONNECTED, DISCONNECTED or WAITING_TO_CONNECT...
	 */
	void xmppRequestStateChange(int newState) {
		int currentState = getConnectionStatus();
		Log.i("xmppRequestStateChange " + statusAsString(currentState) + " => "
				+ statusAsString(newState));
		switch (newState) {
		...
		case XmppManager.WAITING_FOR_NETWORK:
			...
			break;
		default:
			Log.w("xmppRequestStateChange() invalid state to switch to: "
					+ statusAsString(newState));
		}
	...
	/**
	 * Updates the status about the service state (and the statusbar) by sending
	 * an ACTION_XMPP_CONNECTION_CHANGED intent with the new and old state.
	 * needs to be static, because its called by MainService even when xmppMgr
	 * is not created yet
	 * 
	 * @param ctx
	 * @param oldState
	 * @param newState
	 */
	public static void broadcastStatus(Context ctx, int oldState, int newState,
		...
		if (newState == CONNECTED && sXmppManager != null
				&& sXmppManager.mConnection != null) {
			intent.putExtra("TLS",
					sXmppManager.mConnection.isSecureConnection());
			intent.putExtra("Compression",
					sXmppManager.mConnection.isUsingCompression());
		}
		ctx.sendBroadcast(intent);
	}
	
	public static void broadcastReceivedMsg(Context ctx, String message){
	...
	/**
	 * updates the connection status and calls broadCastStatus()
	 * 
	 * @param status
	 */
	private void updateStatus(int status, String action) {
		if (status != mStatus) {
			// ensure _status is set before broadcast, just in-case a receiver
			// happens to wind up querying the state on delivery.
			int old = mStatus;
			mStatus = status;
			mXmppStatus.setState(status);
			mStatusAction = action;
			Log.i("broadcasting state transition from " + statusAsString(old)
					+ " to " + statusAsString(status) + " via Intent "
					+ MainService.ACTION_XMPP_CONNECTION_CHANGED);
	...
	private void updateAction(String action) {
		if (action != mStatusAction) {
			// ensure action is set before broadcast, just in-case a receiver
			// happens to wind up querying the state on delivery.
			mStatusAction = action;
			Log.i("broadcasting new action " + action + "for status "
					+ statusAsString(mStatus) + " via Intent "
					+ MainService.ACTION_XMPP_CONNECTION_CHANGED);
	...
	private void maybeStartReconnect() {
		...
		int timeout = mCurrentRetryCount < 20 ? 5000 * mCurrentRetryCount + 100
				: 1000 * 60 * 5;
		updateStatus(WAITING_TO_CONNECT, "Attempt #" + mCurrentRetryCount
				+ " in " + timeout / 1000 + "s");
		Log.i("maybeStartReconnect scheduling retry in " + timeout
				+ "ms. Retry #" + mCurrentRetryCount);
		mReconnectHandler = new Handler(MainService.getServiceLooper());
	 ...
	 * 
	 * 1. Creates a new XMPPConnection object if necessary 2. Connects the
	 * XMPPConnection 3. Authenticates the user with the server
	 * 
	...
	private void initConnection() {
		...
		assert (!Thread.currentThread().getName()
				.equals(MainService.SERVICE_THREAD_NAME));

		// everything is ready for a connection attempt
		updateStatus(CONNECTING, "");

		// create a new connection if the connection is obsolete or if the old
		// connection is still active
		if (SettingsManager.connectionSettingsObsolete || mConnection == null
	...
	private void onConnectionEstablished(XMPPConnection connection) {
		...
		mConnectionListener = new ConnectionListener() {
			...
			public void connectionClosed() {
				// connection was closed by the foreign host or we have closed
				// the connection
			...
			public void connectionClosedOnError(Exception e) {
				...
				// We update the state to disconnected (mainly to cleanup
				// listeners etc) then schedule an automatic reconnect.
				maybeStartReconnect();
			}
			...
			public void reconnectingIn(int arg0) {
				throw new IllegalStateException(
						"Reconnection Manager is running");
			}
		...
		try {
			...
			XmppOfflineMessages.handleOfflineMessages(mConnection, mSettings
					.getNotifiedAddresses().getAll(), mContext);

			informListeners(mConnection);

		} catch (Exception e) {
			// see issue 126 for an example where this happens because the
			// connection drops while we are in initConnection()
			Log.e("xmppMgr exception caught", e);
			maybeStartReconnect();
			return;
		}

		Log.i("connection established with parameters: con="
				+ mConnection.isConnected() + " auth="
				+ mConnection.isAuthenticated() + " enc="
				+ mConnection.isSecureConnection() + " comp="
				+ mConnection.isUsingCompression());

		// Send welcome message
		if (mSettings.notifyApplicationConnection) {
			Tools.send(
					(mContext.getString(R.string.chat_welcome,
							Tools.getVersionName(mContext))), null, mContext);
		}
	...
	/**
	 * Tries to fully establish the given XMPPConnection Calls
	 * maybeStartReconnect() or stop() in an error case
	 * 
	 * @param connection
	...
	private boolean connectAndAuth(XMPPConnection connection) {
		try {
			updateAction("Connecting to "
					+ sConnectionConfiguration.getServiceName());
			connection.connect();
		} catch (Exception e) {
			Log.w("XMPP connection failed", e);
			if (e instanceof XMPPException) {
				Log.w("XMPP connection failed because of stream error: "
						+ e.getMessage());
			}
			maybeStartReconnect();
			return false;
		...
		try {
			...
			connection.login(mSettings.getLogin(), mSettings.getPassword(),
					Tools.APP_NAME);
		} catch (Exception e) {
			...
			if (e.getMessage() == null
				...
				MainService.displayToast(
						R.string.xmpp_manager_invalid_credentials, null);
				stop();
			}
			return false;
		}
	 ...
	 */
	private XMPPConnection createNewConnection(SettingsManager settings)
			throws XMPPException {
		ConnectionConfiguration conf;

		conf = new ConnectionConfiguration(settings.serverHost.trim(),
	...
	/**
	 * Sends a XMPP Message, but only if we are connected This method is thread
	 * safe.
	 * 
	 * @param message
	 * @param to
	 ...
	 *            will be used
	 * @return true, if we were connected and the message was handled over to
	...
	public boolean send(XmppMsg message, String to) {
		if (to == null) {
			...
			Log.i("Sending message \"" + message.toShortString() + "\" to "
					+ to);
		}
		Message msg;
		...
		try {
			if ((to == null)
					|| (mConnection != null && (XHTMLManager.isServiceEnabled(
							mConnection, to) || !mConnection.isConnected()))) {
		...
		if (isConnected()) {
			// Message has no destination information send to all known
			// resources
			if (to == null) {
			...
			Log.d("Adding message: \""
					...
					+ "\" to offline queue, because we are not connected. Status="
					+ statusString());
			return result;
		}
	}
	...
	public void registerConnectionChangeListener(
			XmppConnectionChangeListener listener) {
